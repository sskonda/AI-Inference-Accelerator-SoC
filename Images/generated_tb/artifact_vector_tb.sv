...
module artifact_vector_tb;
  import soc_pkg::*;
  import accel_pkg::*;

  localparam int unsigned SPM_WORDS = SPM_SIZE_BYTES / DATA_BYTES;
  localparam logic [ADDR_WIDTH-1:0] SRC0_BASE = SPM_BASE_ADDR + 32'h0100;
  localparam logic [ADDR_WIDTH-1:0] SRC1_BASE = SPM_BASE_ADDR + 32'h1100;
  localparam logic [ADDR_WIDTH-1:0] DST_BASE = SPM_BASE_ADDR + 32'h2100;
  localparam logic [ADDR_WIDTH-1:0] SCALAR_ADDR = SPM_BASE_ADDR + 32'h3100;
  ...
  vector_test_top dut (
      ...
      .command_src1_addr(command_src1_addr),
  ...
  task automatic check(input logic condition, input string message);
    if (!condition) begin
      $display("FAIL vector: %s at %0t", message, $time);
      $fatal(1);
    end
  endtask
  ...
  task automatic clear_inputs;
    begin
      command_valid = 1'b0;
      command_opcode = CMD_OP_VECTOR_ADD;
      command_src0_addr = SRC0_BASE;
      command_src1_addr = SRC1_BASE;
  ...
  task automatic run_command(input logic [OPCODE_WIDTH-1:0] opcode,
    ...
    begin
      ...
      command_src0_addr = SRC0_BASE;
      command_src1_addr = (opcode == CMD_OP_VECTOR_SCALE) ? SCALAR_ADDR : SRC1_BASE;
      command_dst_addr = DST_BASE;
      ...
      if (expected_error == ERR_NONE) begin
        check(response_result == length, "success result did not match length");
      end
      response_ready = 1'b1;
      tick();
      response_ready = 1'b0;
      check(!busy, "vector unit did not retire");
    end
  endtask
    ...
    if (!rst_n) begin
      ...
      if (memory_req_valid && memory_req_ready) begin
        if (memory_req_write) begin
          ...
          last_write_data <= memory_req_wdata;
          memory_rsp_rdata <= '0;
        end else begin
        ...
        memory_rsp_error <= fail_next_request || !legal;
        fail_next_request <= 1'b0;
      end
    end
  end
  ...
  initial begin
    trace_fd = $fopen("Images/traces/vector_trace.csv", "w");
    ...
    phase = 8'h10;
    write_element(SRC0_BASE, 0, 16'h0001);
    write_element(SRC0_BASE, 1, 16'h0002);
    write_element(SRC0_BASE, 2, 16'hffff);
    write_element(SRC1_BASE, 0, 16'h0003);
    write_element(SRC1_BASE, 1, 16'h0004);
    write_element(SRC1_BASE, 2, 16'h0002);
    run_command(CMD_OP_VECTOR_ADD, 16'd3, 8'h00, 16'h801, ERR_NONE);
    check(read_element(DST_BASE, 0) == 16'h0004, "vector add element 0 mismatch");
    check(read_element(DST_BASE, 1) == 16'h0006, "vector add element 1 mismatch");
    check(read_element(DST_BASE, 2) == 16'h0001, "vector add element 2 mismatch");
    check(last_write_strobe == 4'h3, "vector partial final strobe mismatch");

    phase = 8'h20;
    reset_dut();
    write_element(SRC0_BASE, 0, 16'h0003);
    write_element(SRC0_BASE, 1, 16'h0004);
    write_element(SRC0_BASE, 2, 16'h0005);
    write_element(SRC0_BASE, 3, 16'h0006);
    write_element(SCALAR_ADDR, 0, 16'h0005);
    run_command(CMD_OP_VECTOR_SCALE, 16'd4, 8'h00, 16'h802, ERR_NONE);
    check(read_element(DST_BASE, 0) == 16'h000f, "vector scale element 0 mismatch");
    check(read_element(DST_BASE, 3) == 16'h001e, "vector scale element 3 mismatch");

    phase = 8'h30;
    reset_dut();
    ready_modulus = 3;
    ready_block_residue = 1;
    write_element(SRC0_BASE, 0, 16'h0001);
    write_element(SRC0_BASE, 1, 16'h0002);
    write_element(SRC0_BASE, 2, 16'h0003);
    write_element(SRC0_BASE, 3, 16'h0004);
    write_element(SRC0_BASE, 4, 16'h0005);
    write_element(SRC1_BASE, 0, 16'h0005);
    write_element(SRC1_BASE, 1, 16'h0004);
    write_element(SRC1_BASE, 2, 16'h0003);
    write_element(SRC1_BASE, 3, 16'h0002);
    write_element(SRC1_BASE, 4, 16'h0001);
    run_command(CMD_OP_VECTOR_ADD, 16'd5, 8'h00, 16'h803, ERR_NONE);
    check(stall_count != 0, "vector backpressure did not create stalls");

    phase = 8'h40;
    reset_dut();
    run_command(CMD_OP_VECTOR_ADD, 16'd0, 8'h00, 16'h804, ERR_DIMENSION);
    check(read_count == 0 && write_count == 0, "invalid vector command issued memory traffic");

    phase = 8'h50;
    reset_dut();
    fail_next_request = 1'b1;
    write_element(SRC0_BASE, 0, 16'h00aa);
    write_element(SRC0_BASE, 1, 16'h00bb);
    run_command(CMD_OP_VECTOR_RELU, 16'd2, 8'h00, 16'h805, ERR_ADDRESS);
    check(write_count == 0, "vector read error should not write destination");

    phase = 8'hff;
    repeat (4) tick();
    $display("PASS artifact_vector_tb add scale backpressure invalid memory_error");
    $fclose(trace_fd);
    $finish;
  end
endmodule
