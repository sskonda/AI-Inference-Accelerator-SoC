...
module artifact_gemm_tb;
  import soc_pkg::*;
  import accel_pkg::*;

  localparam int unsigned SPM_WORDS = SPM_SIZE_BYTES / DATA_BYTES;
  localparam logic [ADDR_WIDTH-1:0] MATRIX_A_BASE = SPM_BASE_ADDR + 32'h0000;
  localparam logic [ADDR_WIDTH-1:0] MATRIX_B_BASE = SPM_BASE_ADDR + 32'h2000;
  localparam logic [ADDR_WIDTH-1:0] MATRIX_C_BASE = SPM_BASE_ADDR + 32'h4000;
  ...
  logic [DIMENSION_WIDTH-1:0] command_m;
  logic [DIMENSION_WIDTH-1:0] command_n;
  logic [DIMENSION_WIDTH-1:0] command_k;
  ...
  logic active_cycle;
  logic stalled_cycle;
  logic [LENGTH_WIDTH-1:0] outputs_completed_event;
  ...
  task automatic check(input logic condition, input string message);
    if (!condition) begin
      $display("FAIL gemm: %s at %0t", message, $time);
      $fatal(1);
    end
  endtask
  ...
  task automatic clear_inputs;
    begin
      command_valid = 1'b0;
      command_opcode = CMD_OP_GEMM;
      command_src0_addr = MATRIX_A_BASE;
      command_src1_addr = MATRIX_B_BASE;
      command_dst_addr = MATRIX_C_BASE;
      command_m = 1;
      command_n = 1;
      command_k = 1;
      command_flags = '0;
  ...
  task automatic run_command(input logic [OPCODE_WIDTH-1:0] opcode,
                             input logic [DIMENSION_WIDTH-1:0] rows,
                             input logic [DIMENSION_WIDTH-1:0] columns,
                             input logic [DIMENSION_WIDTH-1:0] inner,
    ...
    begin
      ...
      command_valid = 1'b0;
      wait_cycle = 0;
      while (!response_valid && wait_cycle < 4000) begin
        tick();
      ...
      tick();
      response_ready = 1'b0;
      check(!busy, "GEMM unit did not retire");
    end
  endtask

  task automatic load_identity_2x2;
    begin
      write_element(MATRIX_A_BASE, 0, 16'd1);
      write_element(MATRIX_A_BASE, 1, 16'd2);
      write_element(MATRIX_A_BASE, 2, 16'd3);
      write_element(MATRIX_A_BASE, 3, 16'd4);
      write_element(MATRIX_B_BASE, 0, 16'd1);
      write_element(MATRIX_B_BASE, 1, 16'd0);
      write_element(MATRIX_B_BASE, 2, 16'd0);
      write_element(MATRIX_B_BASE, 3, 16'd1);
    end
  endtask
  ...
  always @(posedge clk) begin
    if (trace_fd != 0) begin
      $fdisplay(
          trace_fd,
          "%0t,%0d,%0d,%0d,%0d,%0d,%0d,%0d,%0h,%0d,%0h,%0h,%0d,%0d,%0d,%0h,%0h,%0d,%0d,%0d,%0d,%0d",
          $time,
          phase,
          rst_n,
          command_valid,
          ...
          memory_rsp_ready,
          stalled_cycle,
          outputs_completed_event
      );
    end
  end

  initial begin
    trace_fd = $fopen("Images/traces/gemm_trace.csv", "w");
    $fdisplay(trace_fd, "time,phase,rst_n,command_valid,command_ready,busy,done,error,error_code,response_valid,response_error,response_result,memory_req_valid,memory_req_ready,memory_req_write,memory_req_addr,memory_req_wstrb,memory_req_last,memory_rsp_valid,memory_rsp_ready,stalled_cycle,outputs_completed_event");
    ...
    phase = 8'h10;
    load_identity_2x2();
    run_command(CMD_OP_GEMM, 8'd2, 8'd2, 8'd2, 8'h00, 16'hb01, ERR_NONE, 32'd4);
    check(read_element(MATRIX_C_BASE, 0) == 16'd1, "GEMM output C00 mismatch");
    check(read_element(MATRIX_C_BASE, 1) == 16'd2, "GEMM output C01 mismatch");
    check(read_element(MATRIX_C_BASE, 2) == 16'd3, "GEMM output C10 mismatch");
    check(read_element(MATRIX_C_BASE, 3) == 16'd4, "GEMM output C11 mismatch");
    check(write_count == 4 && last_write_count == 1, "GEMM write/final-count mismatch");
    ...
    load_identity_2x2();
    run_command(CMD_OP_GEMM, 8'd2, 8'd2, 8'd2, 8'h00, 16'hb02, ERR_NONE, 32'd4);
    check(stall_count != 0, "GEMM backpressure did not create stalls");

    phase = 8'h30;
    reset_dut();
    run_command(CMD_OP_GEMM, 8'd0, 8'd2, 8'd2, 8'h00, 16'hb03, ERR_DIMENSION, 32'd0);
    check(read_count == 0 && write_count == 0, "invalid GEMM command issued memory traffic");

    phase = 8'h40;
    reset_dut();
    fail_next_read = 1'b1;
    load_identity_2x2();
    run_command(CMD_OP_GEMM, 8'd2, 8'd2, 8'd2, 8'h00, 16'hb04, ERR_ADDRESS, 32'd0);
    check(write_count == 0, "GEMM read error should not write destination");

    phase = 8'h50;
    reset_dut();
    fail_next_write = 1'b1;
    load_identity_2x2();
    run_command(CMD_OP_GEMM, 8'd2, 8'd2, 8'd2, 8'h00, 16'hb05, ERR_ADDRESS, 32'd0);
    check(write_count == 1, "GEMM write error path did not attempt first result write");

    phase = 8'hff;
    repeat (4) tick();
    $display("PASS artifact_gemm_tb multiply backpressure invalid read_error write_error");
    $fclose(trace_fd);
    $finish;
  end
endmodule
